...
my_dog_yd.create_folder(f'PY-142/{breed}')
...
if len(my_dog.sub_breed_list()) == 0: # если у породы нет под-пород
    ...
    my_dog_yd.upload_file(f'PY-142/{breed}/{image_filename}', image_url)
    sleep(1)
    info_file.append(my_dog_yd.file_size(f'PY-142/{breed}/{image_filename}'))
else:
...
my_dog_yd.save_json_yd('info.json', f'PY-142/{breed}/info.json')
